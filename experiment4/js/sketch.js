...
function setup() {
  ...
  cap.size(width, height); // Ensure it uses canvas width/height
  cap.hide();
...
function draw() {
  ...
  // Draw UI
  fill(255);
  rect(width - 25, 22 - 15, 21, 15);
  rect(width - 25, 49 - 15, 21, 15);
  fill(0);
  text(int(bayer_r), width - 5, 19);
  text(int(bayer_threshold), width - 5, 46);
}

function resizeScreen() {
  let centerHorz = canvasContainer.width() / 2; // Adjusted for drawing logic
  let centerVert = canvasContainer.height() / 2; // Adjusted for drawing logic
  console.log("Resizing...");
  resizeCanvas(canvasContainer.width(), canvasContainer.height());
  // Redraw everything based on new size
}
